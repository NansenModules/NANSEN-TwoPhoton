...
%
%   See also nansen.module.twophoton.autosegmentation.flufinder.detect.getBwComponentStats
...
% components
    ...
    if params.FilterByArea % Note: Pretty ad hoc method..
        S = nansen.module.twophoton.autosegmentation.flufinder.detect.refineComponentsByArea(S);
    end
    ...
    while ~finished
        ...
        maskSmall = nansen.module.twophoton.autosegmentation.flufinder.binarize.getRoiMaskFromImage(imSmall, ...
        ...
        if ~isempty(roisOut) && ~isempty(newRoi)
            [~, iB] = nansen.module.twophoton.autosegmentation.flufinder.utility.findOverlappingRois(roisOut, newRoi, 0.75);
            newRoi(iB) = [];
        end
