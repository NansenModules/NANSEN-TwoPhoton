function [roisOut, statOut] = shapeDetection(im, rois, varargin)
    ...
    switch options.Shape
        case 'ring'
            filterWindow = nansen.module.twophoton.autosegmentation.flufinder.filter.makeRingKernel(im, varargin{:});
        case 'disk'
            filterWindow = nansen.module.twophoton.autosegmentation.flufinder.filter.makeDiskKernel(im, varargin{:});
    end
...
% % %     imdataSortedMaxUS = imresize(imdataSortedMax, 4);
    ...
    [masksSmall, s] = nansen.module.twophoton.autosegmentation.flufinder.binarize.findSomaMaskByEdgeDetection(imdata, centerCoords, size(im));

    % Todo: place in fov sized mask
    masks = zeros([size(im), size(masksSmall,3)], 'logical');

    for i = 1:size(masksSmall,3)
        masks(:, :, i) = nansen.module.twophoton.autosegmentation.flufinder.utility.placeLocalRoiMaskInFovMask(...
    ...
    roisOut = nansen.module.twophoton.autosegmentation.flufinder.utility.mergeOverlappingRois(roisOut, overlap);
