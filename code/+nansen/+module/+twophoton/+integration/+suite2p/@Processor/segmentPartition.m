function results = segmentPartition(obj, imArray)
%segmentPartition Segment subpart of ImageStack
%
%   The following is based on the matlab version of suite 2p
%   Todo: Can we use cellDetectionStandalone + sourcery2? Or are they
%         outdated?

    ops = obj.ToolboxOptions;

    if ~isa(imArray,'single'); imArray = single(imArray);  end
    ...
    results = struct('stat', stat, 'ops', ops);
end
